...
impl<'ix_data> TransactionContext<'ix_data> {
    ...
    pub fn new(
        ...
        Self {
            ...
            // InvokeContext::push
            deduplication_maps: vec![u16::MAX; MAX_ACCOUNTS_PER_TRANSACTION.saturating_mul(instruction_trace_capacity.saturating_add(1))],
    ...
    pub fn configure_next_instruction(
        ...
        if self.deduplication_maps.len() < dedup_map_range.end {
            self.deduplication_maps.resize(self.deduplication_maps.len().saturating_add(MAX_ACCOUNTS_PER_TRANSACTION), u16::MAX);
        }
    ...
    pub fn configure_next_instruction_with_ranges(
        &mut self,
        ...
        let new_accounts_len = self.instruction_accounts.len().saturating_add(instruction_accounts_len);
