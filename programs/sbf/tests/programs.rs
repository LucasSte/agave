...
fn process_transaction_and_record_inner(
    ...
    let inner_instructions = inner_instructions.unwrap_or_default(); //.expect("cpi recording should be enabled");
    let log_messages = log_messages.unwrap_or_default(); //.expect("log recording should be enabled");
...
fn authorize_nonce() {
    ...
    for _ in 0..50 {
        ...
        *authorize_ixs.get_mut(2).unwrap() =
        ...
        authority = new_authority.insecure_clone();
    }
